...
public boolean fillWithTiles() {
	for(int i = 0; i < this.tiles.length; i++) {
        ...
        // Puzzle konnte nicht gelöst werden, Tile wird rotiert
		resetPuzzle();
		puzzle[i] = 0;
		tiles[i].rotate();
...
private boolean fillBorders(int tile) {
    ...
    // [...] visited[] wird initialisiert und alle benutzten Teile werden auf true gesetzt
...
Die Methode \emph{fit()} prüft für eine gegebene Stelle im Graphen, ob ein gegebenes Puzzleteil passt. Um zu wissen, welche Seiten bei zwei Teilen verglichen werden müssen, werden die beiden Stellen im Graph voneinander abgezogen. Kommt hier der Wert $-1$ raus, befindet sich das gefundene Teil rechts, bei $1$ links und bei jedem anderen Wert müssen die beiden mittleren Werte verglichen werden. Der Quellcode ist in den Abbildungen \ref{fit} bis \ref{fitEnde} zu finden.
...
\end{lstlisting}
    \caption{Das Tile wird, wenn nötig, geflippt und die Methode beendet, wenn das Tile bereits zweimal rotiert wurde}
    \label{fit}
\end{figure}
...
\end{lstlisting}
\caption{Alle gefundenen Kanten werden geprüft}
\end{figure}
...
\caption{Das Tile wird eingefügt, wenn alles passt. Ansonsten wird es rotiert und erneut probiert}
\label{fitEnde}
\end{figure}
